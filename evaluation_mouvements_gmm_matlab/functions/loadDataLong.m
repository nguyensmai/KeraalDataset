function [ oriMatLong, posMatLong, dataLong ] = loadDataLong( dir, fname, filt, est, rem, ws, resampleSize, trainPose )
%LOADDATALONG Summary of this function goes here
%   load orientation and relative position data for lon sequences with
%   several repetitions
...
[d] = detectEnd(trainPose,data,0);
[maxtab, mintab]=peakdet(d, 0.01);
plot(d);hold on;plot(mintab(:,1),d(mintab(:,1)),'r*');
debut=1;cpt=1;fin=0;
for i=1:length(d)
    diff=(cpt*340)-i;
    prob(i)=exp(-(diff^2)/(2*100^2)) * (1 - (d(i)/3));
    if prob(i)>0.7
        if prob(i)>=prob(i-1)
            fin=i;
        else
            bounds(cpt,1)=debut;bounds(cpt,2)=fin;
            fin=0;
            debut=i;
            cpt=cpt+1;
        end
...
hold on;plot(prob);
dataAll=data;oriMatAll=oriMat;posMatAll=posMat;
for rep=1:size(bounds,1)
    for i=1:length(data)
        data{i}.data = dataAll{i}.data(:,bounds(rep,1):bounds(rep,2));
    end
    oriMat=oriMatAll(:,bounds(rep,1):bounds(rep,2));
    posMat=posMatAll(:,bounds(rep,1):bounds(rep,2));
    % removing start
    if rem==1
    ...
    dataLong{rep}=data;oriMatLong{rep}=oriMat;posMatLong{rep}=posMat;
end
